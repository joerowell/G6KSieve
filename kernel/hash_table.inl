...
// randomness source. This also clears the database. NOT THREAD-SAFE (including
// the randomness calls).
inline void UidHashTable::reset_hash_function(Siever &siever) {
  n = siever.n;
  uid_coeffs.resize(n);
  // Note: this is different from the regular version of G6K. We use a different
  // source of randomness that is only used to reset the uids to prevent these
  // from falling out of sync (the cost of an extra rng is far lower than
  // needing to resynchronise each time we sieve).
  for (unsigned i = 0; i < n; i++)
    uid_coeffs[i] = static_cast<UidCoeffType>(siever.uid_rng());
...
inline std::vector<UidCoeffType> &UidHashTable::get_uid_coeffs() {
  return uid_coeffs;
}

// Compute the uid of x using the current hash function.
inline UidType
UidHashTable::compute_uid(std::array<ZT, MAX_SIEVING_DIM> const &x) const {
  ATOMIC_CPUCOUNT(250);
  return std::inner_product(x.cbegin(), x.cbegin() + n, uid_coeffs.cbegin(),
                            static_cast<UidType>(0));
}
...
inline void UidHashTable::print_state(std::ostream &os) {
  os << "[";
  for (auto &v : db_uid) {
    os << v.size() << " ";
  }
...
inline void UidHashTable::clear_hash_table(Siever &siever) {
  siever.threadpool.run([this](int th_i, int th_n) {
    for (auto j : pa::subrange(DB_UID_SPLIT, th_i, th_n)) {
      db_uid[j].clear();
    }
...
// removes uid from the database without locking. Should only be used
// in single-threaded settings.
inline bool UidHashTable::erase_uid_unsafe(UidType uid) {
  normalize_uid(uid);
  if (uid == 0)
    return false;
...
inline bool UidHashTable::replace_uid(UidType removed_uid, UidType new_uid) {
  ...
  std::unique_lock<std::mutex> lock_new(db_mut[new_uid % DB_UID_SPLIT],
                                        std::defer_lock);
  std::unique_lock<std::mutex> lock_old(db_mut[removed_uid % DB_UID_SPLIT],
                                        std::defer_lock);
  if ((new_uid % DB_UID_SPLIT) == (removed_uid % DB_UID_SPLIT)) {
    ...
    // We simplify by just always locking the smaller index first. -> This
    // avoids deadlocks if done consistently.
    if ((new_uid % DB_UID_SPLIT) < (removed_uid % DB_UID_SPLIT)) {
      lock_new.lock();
      lock_old.lock();
    } else {
  ...
  if (db_uid[new_uid % DB_UID_SPLIT].count(new_uid) !=
      0) // new uid is already present
  {
    //        STATS(++stat_C);
    return false;
  }
  if (db_uid[removed_uid % DB_UID_SPLIT].erase(removed_uid) ==
      0) // try to erase removed_uid. On failure, we bail out.
  {
    return false;
  }
...
inline size_t UidHashTable::hash_table_size_safe() {
  assert(db_uid[0].count(0) == 1);
  size_t num_uid = 0;
  for (unsigned int batch = 0; batch < DB_UID_SPLIT; ++batch) {
    std::lock_guard<std::mutex> lock(db_mut[batch]);
    num_uid += db_uid[batch].size();
  }
  return num_uid - 1;
}
