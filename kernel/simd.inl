...
inline Simd::SmallVecType Simd::m128_add_epi64(const SmallVecType a,
  ...
  SmallVecType result;
  static_assert(sizeof(result) == sizeof(c),
                "Error: sizeof(SmallVecType) != sizeof(Vec2q)");
...
inline Simd::VecType Simd::m256_permute4x64_epi64(const VecType a) {
  ...
  // NOTE: the memcpying here is a type-punning guard. Technically reinterpret
  // casting here would be UB (GCC has an extension for this, but Clang
  // doesn't). The compiler knows that this isn't actually an operation that it
  // needs to do: the backend optimises out the memcpys (see
  // https://godbolt.org/z/zxKvnKdeP: the code for this function compiles down
  // to a vectorised permutation).
  Vec4q tmp_a;
...
inline int Simd::m256_testz_si256(const VecType a, const VecType b) {
  ...
  // To get around this, we just copy into a fixed-size array and compare
  // against the all zero. Again, the copy here is a type-punning guard.
  const auto res = a & b;
...
template <int pos> inline Simd::VecType Simd::m256_slli_epi16(const VecType a) {
#ifdef HAVE_AVX2
...
inline Simd::SmallVecType Simd::m128_shuffle_epi8(const SmallVecType in,
  ...
  static_assert(sizeof(in_arr) == sizeof(SmallVecType),
                "Error: wrong array size for copy");
...
inline Simd::VecType Simd::m256_shuffle_epi8(const VecType in,
  ...
  static_assert(sizeof(mask_as_arr) == sizeof(in),
                "Error: wrong vector size for copy");
  ...
  const auto res_1 = Simd::m128_shuffle_epi8(first, first_mask);
  const auto res_2 = Simd::m128_shuffle_epi8(last, last_mask);
