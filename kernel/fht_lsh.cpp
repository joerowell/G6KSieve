...
// https://github.com/lducas/AVX2-BDGL-bucketer commit
// 630c2286a440fae1eddd9f90341ff2020f18b614 This has since been modified: this
// version can be found at: https://github.com/joerowell/gcc-bucketer commit
// 868c3ef

// insert_in_maxs. Given a hash value val, insert it into maxs along with its
// index.
inline void FastHadamardLSH::insert_in_maxs(int32_t *const maxs,
                                            const int16_t val,
                                            const int32_t index) {

  // Firstly if the value isn't bigger than our minimum then we exit.
  // Note that we offset by one - this is because we pack the value next to its
  // index
  if (std::abs(val) < maxs[2 * (multi_hash - 1)]) {
    return;
  }

  // If we're only hashing against one bucket then pack the value and its index
  // in maxs (maxs is of size 2) and return
  int i = multi_hash - 1;
  if (!i) {
    maxs[0] = std::abs(val);
  ...
  // As we deal with pairs, we need to decrement this here to make sure we start
  // in the right position
  i--;

  // Starting at the end of xs, we shift all of the existing hash pairs along
  // This means that we end up in the right place for inserting this value and
  // its index
...
inline void FastHadamardLSH::insert_in_maxs_epi16(int32_t *const maxs,
                                                  const int i_high,
  ...
  // Set up the iteration condition - we want a value of at max 16 (as it's a
  // 256-bit vector), but codesize-i_high encodes what the length should be.
 ...
 * @param res - a pointer to receive the multi_hash many (coefficient, hashes)
 pairs (pairs are written next to each other in this array of size 2*multi_hash)
 */
template <int regs_>
void FastHadamardLSH::hash_templated(const int16_t *const vv,
                                     int32_t *const res) {
  ...
  // We grab the appropriate tailmask & set up the randomness for the
  // permutations
  ...
  // If we have more than 2 registers to operate on then we apply the Hadamard
  // transform on 32 entries at a given time However, if we've got only 2
  // registers to operate on then we need to use different permutations and we
  // only apply the Hadamard transform on 16 entries at a given time.
  if (regs_ > 2) {
    // h0 and h1 contains the Hadamard transforms of v[0] and v[1] on each
    // iteration respectively.
    Simd::VecType h0, h1;
    // Permute the 16-bit integers in the matrix
    Simd::m256_permute_epi16<regs_>(v, prg_state,
                                    reinterpret_cast<Simd::VecType>(tailmask),
                                    aes_key, &extra_state);

    // Now we apply the Hadamard transformations and permutations, inserting the
    // scores in to the res array (note that these are double-packed)
 ...
 * Note that this function is separate from the different "hash_templated"
 * functions - that's because we apply some normalisation and safety checks
 * first.
 * @param v - a pointer to the vector which is hashed
 * @param coeff - a pointer to receive the multi_hash many coefficient of the
 * selected hash values
 * @param hashes - a pointer to receive the multi_hash many hash values
 */
void FastHadamardLSH::hash(const float *const v, float *const coeff,
                           int32_t *const hashes) {
  ...
  // This is the 0-vector case - in that case, we reply with the default
  // response
  ...
  // the largest sum over 32 values by sorting them), but this may be enough
  // already
  ...
  // With this finished, we have to renormalise the coefficients of the results
  // array We place the results of this into the output parameters Note that we
  // double pack the res array so that we can split out nicely
  renorm = 1. / renorm;
  for (unsigned i = 0; i < multi_hash; ++i) {
...
/*
 * insert_in_maxs. Given a score, insert that score into the right position in
 * scores, whilst also inserting the index into the right position in the
 * indices array. */
inline bool ProductLSH::insert_in_maxs(float *const scores,
 ...
 */
template <>
void ProductLSH::hash_templated<3>(const float *const vv, int32_t *const res) {
  ...
  // This memset is inserted to prevent some compilers from complaining about
  // initialising variable-length arrays. This is because variable-length arrays
  // are a compiler extension and not endorsed by the C++ standard. Note that
  // this call to memset actually appears to generate better code on more recent
  // versions of gcc. See https://godbolt.org/z/vGh9c69br for an example
  assert(multi_hash > 0);
  assert(multi_hash_block > 0);
  float c[multi_hash];
  ...
  for (unsigned i0 = 0; i0 < multi_hash_block; ++i0) {
    ...
    for (; i1 < multi_hash_block; ++i1) {
      ...
      // if the inner loop failed on i2 = 0 then break - we move on to the next
      // coefficient
      if (!i2)
        break;
    }
...
/*
 * hash_templated. This function is a specialisation of hashing vv against 2
 * sub-code blocks.
 */
template <>
void ProductLSH::hash_templated<2>(const float *const vv, int32_t *const res) {
  ...
  for (unsigned i0 = 0; i0 < multi_hash_block; ++i0) {
    ...
    unsigned i1 = 0;
    for (; i1 < multi_hash_block; ++i1) {
      // Transform the inputs - apply the sign of the first hash against the
      // second Then do the same as above, but offset the coefficient by c1[i1].
      int32_t h1_ = sign0 * h1[i1];
      h1_ = 2 * (std::abs(h1_) - 1) + (h1_ > 0);
      h1_ = 2 * codesizes[1] * h0_ + h1_;
      float c1_ = c0_ + c1[i1];
    ...
    // If we fail to insert on the first iteration then break from the outer
    // loop - we're done
    if (!i1)
      break;
  }
...
template <>
void ProductLSH::hash_templated<1>(const float *const vv, int32_t *const res) {
...
/**
 * hash. Accepts a vector, v, and hashes it against all of the relevant
 * subcodes.
 * @param v - a pointer to the vector which is hashed
...
void ProductLSH::hash(const float *const v, int32_t *const res) {
  // Firstly we apply the permutation - we apply the same permutation to every
  // input vector. The sign and permutation vectors are built in the constructor
  // for this class.
  float vv[n];
  for (size_t i = 0; i < n; ++i) {
