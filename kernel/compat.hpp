...
/**
  This file defines some compatibility macros / workarounds for C++-14 / C++-17
  features. Notably, it defines the namespace mystd::, which contains
  alternative implementations for some (very simple) C++14 and beyond std::
  functions/typedefs/etc. If we detect that the C++ implemtation has these, the
  corresponding mystd:: variants are just aliased to the std:: ones, otherwise
  we implement them on our own.
*/
...
/**
  CPP14CONSTEXPR is used to declare functions as constexpr, provided the
  (considerable!) relaxations to the constexpr specifier from C++14 are
  supported.

  CPP17CONSTEXPRIF is used to support C++17 if constexpr. If unsupported, falls
  back to a normal if. Note that the main point of if constexpr(expr){ foo() }
  is that expr might depend on a template argument and if expr is false, foo
  does not need to even compile (with some caveats). This feature would indeed
  be extremely useful and simplify *a lot* of code. (The workaround is to define
  a class template X, templated by a bool b. Specialize X for b==true with
  static member function f that implements foo and specialize X for b==false
  with a member function that does nothing. Then we can call X<epxr>::f(arg),
  passing required arguments arg. This is the state-of-the-art in C++11, which
  is extremely annoying and clutters the code.) Unfortunately, to support the
  fallback to normal if, we cannot use CPP17CONSTEXPRIF that way. Instead,
  CPP17CONSTEXPRIF is supposed to be used for if's that should depend on
  compile-time expressions in order to trigger compiler errors if this is not
  the case and to document intent. Efficiency-wise, there is no benefit if the
  compiler is able to elimate if(false){ ... } code (which seems a safe
  assumption).
*/

// Note that undefined symbols in an #if get replaced by 0, so this works even
// if
...
/**
  ...
  We want a lot of templates to only be instantiatable if the template
  parameters satify certain boolean constraints (e.g. "Is a lattice Point", "Is
  an integer", "Is an Iterator"...) Lacking the "requires" keyword, we use
  std::enable_if and SFINAE. To improve readability and making the intent
  clearer, we define some macros TEMPL_RESTRICT_DECL, TEMPL_RESTRICT_IMPL Usage:
  template<class Integer, TEMPL_RESTRICT_DECL(std::is_integral<Integer>::value)>
  some_declaration; template<class Integer,
  TEMPL_RESTRCIT_IMPL(std::is_integral<Integer>::value)> some_definition;

  This restricts the template to only be instantiatable if the argument to
  TEMPL_RESTRICT_DECL/IMPL is convertible to boolean true at compile time. Use
  as the last template argument. (To use this to restrict variadic templates,
  you should know what you are doing)

  The variants TEMPL_RESTRICT_DECL2 and TEMPL_RESTRICT_IMPL2 are similar, but
  take a comma-separated list of *types* T_1,T_2,... encapsulating a boolean
  value instead. (i.e. types with static constexpr bool T_i::value defined --
  This is compatible with all such types and manipulators from std:: ). It
  restricts to the AND of these types.

  The difference between the DECL and IMPL variants is that you need to use DECL
  in declarations or declarations + definitions, whereas you need to use IMPL in
  definitions of previously declared template member functions (i.e. when
  declaring a member function inside a class, use DECL. If you use a separate
  file to then define the function, you need to use IMPL -- This is because DECL
  sets default parameters and IMPL does not)

  NOTE: It is generally preferable to just static_assert the condition inside
  the class template / function, provided this is an option. This gives better
  error messages.

        The purpose of these macros is to restrict the viable candidates for
  overload resolution, *NOT* to assert that some condition is satisfied. Use
  cases are when you have several overloads or specializations. A typical use
  case is for templated overloads of operators where you might conflict with
        other pre-existing overloads or overloads that might be added later.

  NOTE: The TEMPL_RESTRICT_DECL/IMPL(arg) macros only take ONE "real" argument.
  However, since these are macros and the C preprocessor does not recognize
  template angle-brackets < > as delimeters, commas inside template parameter
  lists can (and do!) often get misparsed. For this reason, it is declared as a
  variadic macro with an arbitrary number of arguments.
*/

/**
  NOTE: If you get an compiler error "no type named "type" in
  std::enable_if<false,int>, then you are using TEMPL_RESTRICT_* wrongly. Due to
  some choices in the C++ standard the condition argument(s) to TEMPL_RESTRIC_*
  must evaluate to true for at least one possible choice of template arguments.

        In the important case of member functions templates of class templates,
  this means that for each instantiations of the class (i.e. class template
  parameters fixed), there has to be a set of template arguments for the
  function, such that this holds.
        ...
        class Myclass
        {
          template<class dummy = ClassArg,
  TEMPL_RESTRICT_DECL2(Condition<dummy>)> foo ()
          {
            // you might want to
  static_assert(std::is_same<dummy,ClassArg>::value, "");
            // because the caller might set the dummy template parameter
  explitly by calling
            // foo<Bar>()
            // (If foo has other template paramters as well, there could be a
  mix-up)
            ...
          }
        };
...
/**
  Replacements for C++14 (and beyond) standard library features that are missing
  in C++11. Since these are all documented in the C++ standard, we do not give
  much explanation and refer to the C++ - documentation (which is much better,
  anyway). In general, mystd::foo mimicks std::foo.
*/

namespace mystd {
// some often-used shorthands to avoid having to use typename
// (my)std::foo<Bar>::type we use (my)std::foo_t<Bar> instead completely
// identical to the corresponding std:: definitions that were added to C++
// post-C++11
// clang-format off
...
// std::max is not (and can't be) constexpr until C++14 (where the def. of
// constexpr was relaxed). This version is always constexpr, but does not
// support custom comparators or initializer lists. Hence, this differs from the
// std:: version, which is why we don't call it mystd::max
  ...
  inline constexpr ENUMNAME OPNAME(ENUMNAME const lhs, ENUMNAME const rhs) {   \
    return static_cast<ENUMNAME>(                                              \
        static_cast<std::underlying_type<ENUMNAME>::type>(lhs)                 \
            OP static_cast<std::underlying_type<ENUMNAME>::type>(rhs));        \
  }

#define MYSTD_DECLARE_BIT_OPS_FOR_ENUM2(OP2, OPNAME, ENUMNAME)                 \
  inline ENUMNAME &OPNAME(ENUMNAME &lhs, ENUMNAME const rhs) {                 \
    lhs = static_cast<ENUMNAME>(                                               \
        static_cast<std::underlying_type<ENUMNAME>::type>(lhs)                 \
            OP2 static_cast<std::underlying_type<ENUMNAME>::type>(rhs));       \
    return lhs;                                                                \
  }

#define ENABLE_BITOPS_FOR_ENUM(ENUMNAME)                                       \
  static_assert(std::is_enum<ENUMNAME>::value == true,                         \
                #ENUMNAME " is not an enumeration type.");                     \
  ...
  inline constexpr ENUMNAME operator~(ENUMNAME const arg) {                    \
    return static_cast<ENUMNAME>(                                              \
        ~static_cast<std::underlying_type<ENUMNAME>::type>(arg));              \
  }
...
struct cacheline_padding_t {
  uint64_t pad[16];
};
// _ ## NOTE: added to interfere less with auto-completion of my editor
#define CACHELINE_VARIABLE(type, name)                                         \
  cacheline_padding_t _##name##prepad;                                         \
  type name;                                                                   \
  cacheline_padding_t _##name##postpad;
#define CACHELINE_VARIABLE2(type, name, init)                                  \
  cacheline_padding_t _##name##prepad;                                         \
  type name init;                                                              \
  cacheline_padding_t _##name##postpad;
...
// We occasionally want to pad so that we do not end up
// with false sharing. The cacheline_padding_t struct simply adds a trailing
// 128 bytes, but this is overly pessimistic in some settings, as
// it can round up the size of certain structures unnecessarily.
// In most settings, it suffices to allocate objects so that they have the
// alignment of 2 * the size of a cache line: this has the same effect.
// Q: Shouldn't this be 1 * CACHELINE_ALIGNMENT? A: No, because certain
// platforms will do aligned prefetching across multiple cachelines. See e.g.
// https://cdrdv2-public.intel.com/671488/248966-Software-Optimization-Manual-R047.pdf
// E.2.5.4 on "Spatial Prefetcher".

#ifdef __cpp_lib_hardware_interference_size
#define CACHELINE_ALIGNMENT std::hardware_destructive_interference_size
#else
