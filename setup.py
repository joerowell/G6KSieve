...
def read_compiler(filename, field, sep):
    # This is separate to handle potential lists of arguments
    out = read_from(filename, field, sep)
    if isinstance(out, str):
        return out
    elif isinstance(out, list):
        return out[0]
    else:
        raise ValueError("Did not understand format of", field)

# Make sure that the environment variable is picked up (just in case we're using MPI). 
os.environ["CXX"] = read_compiler("Makefile", "CXX", "=")
os.environ["CC"] = read_compiler("Makefile", "CC", "=")
