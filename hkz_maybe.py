...
def hkz_kernel(arg0, params=None, seed=None):
    ...
    workout(
        g6k, tracer, 0, n, pump_params=pump_params, verbose=verbose, **workout_params
    )
...
def hkz():
    ...
    fmt = "{name:50s} :: n: {n:2d}, cputime {cputime:7.4f}s, walltime: {walltime:7.4f}s, flast: {flast:3.2f}, |db|: 2^{avg_max:.2f}, extra_memory(MB): {extra_memory:.2f}, ratio: {extra_used_ratio:.2f}, bandwidth (MB): {sieving_bandwidth:.2f}, center bandwidth (MB): {center_bandwidth:.2f}, bucket bandwidth(MB): {bucket_bandwidth:.2f}"
