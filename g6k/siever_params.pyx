...
from decl cimport MAX_SIEVING_DIM, DistSieverType
...
cdef class SieverParams(object):
    ...
    def __init__(self, **kwds):
        ...
        kwds["is_cython"] = True # We are always a cython node. 
        ...
        if "scale_factor" not in kwds:
            kwds["scale_factor"] = 1
        if "bucket_batches" not in kwds:
            kwds["bucket_batches"] = 1
        if "scratch_buffers" not in kwds:
            kwds["scratch_buffers"] = 1            
        read_only = False
    ...
    cpdef _set(self, str key, object value):
        ...
        if key == "reserved_n":
            ...
            self._core.is_cython = value
        elif key == "topology":
            if value == 0 or value == "Null":
                self._core.topology = <DistSieverType>(0)
            elif value == 1 or value == "ShuffleSiever":
                self._core.topology = <DistSieverType>(1)
            else:
                raise ValueError("topology %s not understood"%value)
        elif key == "dist_threshold":
            self._core.dist_threshold = value
        elif key == "scale_factor":
            self._core.scale_factor = value
        elif key == "bucket_batches":
            self._core.bucket_batches = value
        elif key == "scratch_buffers":
            self._core.scratch_buffers = value
        else:
    ...
    cpdef _get(self, str key):
        if key == "reserved_n":
            ...
            return self._core.is_cython
        elif key == "topology":
            return <int>self._core.topology
        elif key == "dist_threshold":
            return self._core.dist_threshold
        elif key == "scale_factor":
            return self._core.scale_factor
        elif key == "bucket_batches":
            return self._core.bucket_batches
        elif key == "scratch_buffers":
            return self._core.scratch_buffers
        else:
