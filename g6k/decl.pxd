...
cdef extern from "../kernel/siever.h" nogil:
    ...
    cdef cppclass SieveStatistics:
        ...
        bool collect_statistics_gbl_max_len_var
        double get_stats_gbl_max_len_variance() const

        bool collect_statistics_gbl_sat_var
    ...
    cdef cppclass SieverParams:
        ...
        # MPI stuff
        bool is_cython
        DistSieverType topology
        unsigned int dist_threshold
        unsigned int scale_factor
        unsigned int bucket_batches
        unsigned int scratch_buffers
    ...
    cdef cppclass Siever:
        ...
        void shrink_left(unsigned int lp, bool down_sieve)
        ...
        void gso_update_postprocessing(const unsigned int l_, const unsigned int r_, long* M, const bool down_sieve)
        ...
        # Distributed sieving CPU time function.
        long double get_cpu_time()

        # Distributed sieving extra memory used.
        # N.B The value returned here is in bytes. 
        uint64_t get_extra_memory_used()
        double db_to_extra_ratio()

        # These return the following information: the total number of messages sent during sieving,
        # the total number of bytes used to send those messages, the amount of bytes used for sending the initial
        # centers (if applicable), and the amount of bytes used for sending all bucketing replies.
        uint64_t get_sieving_messages_used()
        uint64_t get_sieving_bandwidth_used()
        uint64_t get_center_bandwidth_used()
        uint64_t get_buckets_bandwidth_used()

        
        

cdef extern from "../kernel/siever.h" namespace "DistSieverType" nogil:
   cdef DistSieverType Null
   cdef DistSieverType ShuffleSiever
