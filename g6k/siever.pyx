...
cdef class Siever(object):
    ...
    @property
    def _stat_GBL_max_len_variance(self):
        return self._core.statistics.collect_statistics_gbl_max_len_var

    @property
    def _stat_c_GBL_max_len_variance(self):
        return self._core.statistics.get_stats_gbl_max_len_variance()

    @property
    def _stat_GBL_saturation_count_variance(self):
        return self._core.statistics.collect_statistics_gbl_sat_var

    @property
    def _stat_c_GBL_saturation_count_variance(self):
        return self._core.statistics.get_stats_gbl_sat_variance()
    ...
    # Note: Buckets == Filtered lists (the terminology depends on the algorithm)
    all_statistics = {
        ...
        "GBL_max_len_variance"  : [163, "GBLMLV:", "Population variance of GBL_max_len across all processes",                      {"bgj1"}],
        "GBL_saturation_count_variance" : [164, "GBLSCV:", "Population variance of GBL_saturation_count across all processes",     {"bgj1"}],
    }
    ...
    def shrink_left(self, lp=1, down_sieve = False):
        """
        ...
        assert(self.initialized)
        sig_on()
        self._core.shrink_left(lp, down_sieve)
        sig_off()
    ...
    def insert(self, kappa, v, should_redist = True):
        """
        ...
        :param should_redist. True if we need the distributed sieving layer to rebalance the database, false otherwise.
        """
        assert(self.initialized)
        assert(len(v) == self.r)
        m = self.full_n
        ...
        sig_on()
        self._core.gso_update_postprocessing(new_l, self._core.r, <long*>T.data, should_redist)
        sig_off()
    ...
    def insert_best_lift(self, scoring=(lambda index, nlen, olen, aux: True), aux=None):
        ...
        if aux is not None:
            self.insert(-best_i, best_v, not (aux.phase == "down" and aux.down_sieve == False))
        else:
    ...
    def get_cpu_time(self):        
        return self._core.get_cpu_time()

    def get_extra_memory_used(self):
        return self._core.get_extra_memory_used()

    def get_sieving_messages_used(self):
        return self._core.get_sieving_messages_used()
    
    def get_sieving_bandwidth_used(self):
        return self._core.get_sieving_bandwidth_used()

    def get_center_bandwidth_used(self):        
        return self._core.get_center_bandwidth_used()

    def get_buckets_bandwidth_used(self):
        return self._core.get_buckets_bandwidth_used()

    def get_db_to_extra_ratio(self):        
        return self._core.db_to_extra_ratio()
