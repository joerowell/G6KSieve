...
def load_svpchallenge_and_randomize(
    n, s=None, seed=None, verbose=True, float_type="double"
):
    """
    ...
    if os.path.isfile(filename) is False:
        logging.info(
            "Did not find '{filename}', downloading ...".format(filename=filename)
        )
        r = requests.post(
...
def load_prebkz(n, s=0, blocksize=40):
    ...
    if os.path.isfile(filename) is False:
        ...
        print(
            "Did not find '{filename}'. Creating and reducing".format(filename=filename)
        )
        print("created, ", end=" ")
        sys.stdout.flush()
        A = LLL.reduction(A)
...
def extract_max_and_avg(stats, key: str):
    """
    Given a list of traces, find the average and maximum of "key" in traces.
    :param stats: a list of traces of type ``Node``.
    :param key: the key to search for
    """
    max_key = Accumulator(0, repr="avg", count=False)
    for stat in stats:
        max_key += stat.accumulate(
            key,
            filter=lambda node: SieveTreeTracer.is_sieve_node(node.label),
            repr="max",
        ).max
    return max_key.avg, max_key.max


def extra_usage(stats):
    """
    Given a list of traces, find the average and the maximum of "extra_usage" in traces.
    :param stats: a list of traces of type ``Node``
    """
    return extract_max_and_avg(key="extra_used_ratio", stats=stats)


def sieving_bandwidth(stats):
    """
    Given a list of traces, find the average and the maximum of "sieving_bandwidth" in traces.
    :param stats: a list of traces of type ``Node``
    """
    return extract_max_and_avg(key="sieving_bandwidth", stats=stats)


def center_bandwidth(stats):
    """
    Given a list of traces, find the average and the maximum of "center_bandwidth" in traces.
    :param stats: a list of traces of type ``Node``
    """
    return extract_max_and_avg(key="center_bandwidth", stats=stats)


def bucket_bandwidth(stats):
    """
    Given a list of traces, find the average and the maximum of "bucket_bandwidth" in traces.
    :param stats: a list of traces of type ``Node``
    """
    return extract_max_and_avg(key="bucket_bandwidth", stats=stats)


def extra_memory(stats):
    """
    Given a list of traces, find the average and the maximum "extra_memory" and
    the maximum "extra_memory" for the traces.
    :param stats: a list of traces of type ``Node``
    """
    return extract_max_and_avg(key="extra_memory", stats=stats)
...
def db_stats(stats):
    ...
    """
    max_and_avg = extract_max_and_avg(key="|db|", stats=stats)
    return log(max(1, max_and_avg[0]), 2), log(max(1, max_and_avg[1]), 2)
...
def load_lwe_challenge(n=40, alpha=0.005):
    ...
    if not os.path.isfile(filename):
        url = (
            "https://www.latticechallenge.org/lwe_challenge/challenges/"
